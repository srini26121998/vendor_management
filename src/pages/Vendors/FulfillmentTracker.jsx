                    ...
                    <div className="min-w-max">
                        ...
                        <div className="flex gap-4 mb-4">
                            {PIPELINE_STAGES.map((stage, i) => (
                                <div key={i} className="w-56 flex-shrink-0">
                                    <div className="flex items-center gap-2 px-4 py-2.5 rounded-lg text-[11px] font-bold text-white shadow-sm"
                        ...
                        <div className="flex gap-4">
                            {PIPELINE_STAGES.map((stage, si) => {
                                const stagePOs = purchaseOrders.filter(po => po.stage === si);
                                return (
                                    <div key={si} className="w-56 flex-shrink-0 space-y-3">
                                        {stagePOs.map(po => {
                                            ...
                                            return (
                                                <div key={po.id} onClick={() => navigate(`/vendors/purchase-orders/${po.id}`)}
                                                    className="p-4 bg-white rounded-xl border border-gray-100 shadow-sm cursor-pointer hover:shadow-md transition-all group">
                                                    <div className="font-mono text-[11px] text-blue-600 font-bold mb-1.5 flex justify-between">
                                                        <span>{po.poNumber}</span>
                                                        <span className={`px-1.5 py-0.5 rounded text-[10px] ${sla === 'red' ? 'bg-rose-50 text-rose-600' : sla === 'amber' ? 'bg-amber-50 text-amber-600' : 'bg-emerald-50 text-emerald-600'}`}>
                                                            {daysInStage}d
                                                        </span>
                                                    </div>
                                                    <div className="text-[13px] font-semibold text-gray-700 line-clamp-1 mb-3 group-hover:text-blue-600 transition-colors">{po.vendor}</div>
                                                    ...
                                                    <div className="mt-3 pt-3 border-t border-gray-50 flex items-center gap-2 text-[10px] text-gray-400 font-medium">
                                                        <span>📅 Delivery: {formatDate(po.delivery)}</span>
                                                    </div>
                                                </div>
                                            );
                                        })}
                                        {stagePOs.length === 0 && (
                                            <div className="border border-dashed border-gray-200 rounded-xl p-6 text-center text-[11px] font-bold text-gray-300 uppercase ">No Items</div>
                                        )}
                                    </div>
                                );
                            })}
                ...
                <div className="space-y-6">
                    ...
                    <div className="flex items-center justify-between py-2 px-2">
                        <div className="text-[12px] font-medium text-gray-400">
                            Showing <span className="font-bold text-gray-700">{(currentPage - 1) * itemsPerPage + 1}</span> to <span className="font-bold text-gray-700">{Math.min(currentPage * itemsPerPage, sortedData.length)}</span> of <span className="font-bold text-gray-700">{sortedData.length}</span> entries
                        </div>
